...
namespace Celeste
{
    ...
    [CustomEntity("GooberHelper/RefillBumper")]
	public class RefillBumper : Entity
	{
		// Token: 0x06000DC7 RID: 3527 RVA: 0x00030D6C File Offset: 0x0002EF6C
		public RefillBumper(Vector2 position, Vector2? node)
			: base(position)
		{
		...
		public RefillBumper(EntityData data, Vector2 offset)
		...
		public override void Update()
		{
			...
			else if (base.Scene.OnInterval(0.05f))
			{
				...
				ParticleType particleType = (this.fireMode ? Bumper.P_FireAmbience : Bumper.P_Ambience);
				float num2 = (this.fireMode ? (-1.5707964f) : num);
				float num3 = (float)(this.fireMode ? 12 : 8);
				base.SceneAs<Level>().Particles.Emit(particleType, 1, base.Center + Calc.AngleToVector(num, num3), Vector2.One * 2f, num2);
			}
		}
		...
		private void OnPlayer(Player player)
		{
			...
			else if (this.respawnTimer <= 0f)
			{
				...
				// Water thing = this.CollideFirst<Water>(this.Position);

				// if(thing != null) { 
				// 	Logger.Log(LogLevel.Info, "gooberhelper", thing.Position.ToString());
				// } else {
				// 	Logger.Log(LogLevel.Info, "gooberhelper", "its null");
				// }
				

                // List<Entity> entities = this.CollideAll<Entity>(this.Position);

                // Logger.Log(LogLevel.Info, "gooberhelper", "hi");

                // foreach(Entity entity in entities) {
                //     Logger.Log(LogLevel.Info, "gooberhelper", entity.GetType().Name);
                // }
			}
		}
